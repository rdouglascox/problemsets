...
module MakePS.MakeTranslationTest (mktranstestg,mktransteststring) where
...
mktransteststring :: IO (String, String)
mktransteststring = do
...
mktranstestg :: RandomGen g => g -> Int -> IO ()
mktranstestg g n  = do
         let (q,qa) = transtest g
         renderFile ("transtest" ++ "-" ++ show n ++ ".tex") (ps11a qa n) -- render answers to tex
              where (g1,g2) = split g
...
ps11pa :: Int -> LaTeX
ps11pa n = docSettings <> title "Translations Test" <> author "" <> date (fromString $ "#" ++ show n)
